...
//        sumProgram();
    ...
    private static void sumProgram(){
        try {
            ...
            for (int nrThreads = 1; nrThreads <= sumResultSize; nrThreads++) {
                ...
                ThreadPoolExecutor sumThreadPool = (ThreadPoolExecutor) Executors.newFixedThreadPool(nrThreads);
                ...
                for (int j = 1; j <= nrThreads; j++) {
                    ...
                    sumThreadPool.execute(new SumExecutor(sumMatrixes.getMatrix1(), sumMatrixes.getMatrix2(), result, sumMatrixes.getM1(), startElement, startElement + threadShare - 1));
                ...
                sumThreadPool.shutdown();
                sumThreadPool.awaitTermination(5, TimeUnit.MINUTES);
    ...
    private static void productProgram(){
        try {
            ...
            for (int nrThreads = 1; nrThreads <= prodResultSize; nrThreads++) {
                ...
                //start threads
                Instant startTime = Instant.now();
                ThreadPoolExecutor prodThreadPool = (ThreadPoolExecutor) Executors.newFixedThreadPool(nrThreads);
                ...
                for (int j = 1; j <= nrThreads; j++) {
                    ...
                    prodThreadPool.execute(new ProductExecutor(prodMatrixes.getMatrix1(), prodMatrixes.getMatrix2(), result, prodMatrixes.getM2(),
                            prodMatrixes.getM1(), startElement, startElement + threadShare - 1));
                ...
                prodThreadPool.shutdown();
                prodThreadPool.awaitTermination(5, TimeUnit.MINUTES);
                //calculate elapsed time
                Instant endTime = Instant.now();
                Duration timePassed = Duration.between(startTime, endTime);
                String timeUnit = timePassed.getSeconds() == 0 ? timePassed.toMillis() + " milliseconds." : timePassed.getSeconds() + " seconds.";
    ...
    public static Read2MatrixResult readTwoMatrixesFromFile(String fileName) throws IOException {
